...
duplicates.get("/", async (c) => {
  try {
    ...
    // 새로운 임계값 시스템 사용
    const thresholds = deserializeThresholds(c.req.query());

    // 하위 호환성을 위해 기존 threshold 파라미터도 지원
    const legacyThreshold = c.req.query("threshold");
    if (legacyThreshold && !c.req.query("imagePerceptual")) {
...
// GET /api/duplicates/progress - 진행 상황 조회 (필요시 구현)
duplicates.get("/progress", async (c) => {
  // 향후 진행 상황 추적이 필요한 경우 구현
  return c.json({ progress: 100, status: "completed" });
});
