...
export function deserializeThresholds(
  ...
  try {
    // URLSearchParams 또는 일반 객체 모두 처리
    ...
    return {
      image: {
        perceptual: parseInt(getValue("imagePerceptual") || "85"),
        exact: getValue("imageExact") === "true",
      },
      video: {
        visual: parseInt(getValue("videoVisual") || "70"),
        duration: parseInt(getValue("videoDuration") || "20"),
        exact: getValue("videoExact") === "true",
      },
      global: {
        minFileSize: parseInt(getValue("minFileSize") || "1024"),
        skipSmallFiles: getValue("skipSmallFiles") === "true",
      },
    };
  } catch (error) {
